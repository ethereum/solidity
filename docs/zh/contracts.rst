...
状态变量可以声明为常量``constant``。在此状况下，其必须在编译时表达式是一个常量并从其赋值。任何访问存储、区块数据(如 ``now``, ``this.balance`` 或 ``block.number`` )、执行数据 ``msg.gas`` )或对外部合约的调用等都操作都不可以。可能会对内存分配造成副作用的表达式，但对其它内存对象可能没有副作用。允许内置函数 ``keccak256``, ``sha256``, ``ripemd160``, ``ecrecover``, ``addmod``, ``mulmod`` (即使它们调用外部合约)。

允许内存分配器副作用的原因在于：可以构建复杂的对象(如:lookup-tables)。此特性目前还不是完全可用。

编译器不保留这些变量的存储空间，并且每个事件都由相应的常量表达式替换(其可能被优化器计算为单个值).

不是所有常量的类型都实现了。 唯一支持的类型是值类型和字符串。

译注：在数组和结构体类型上仍然不可以这样做，映射类型也不可以
