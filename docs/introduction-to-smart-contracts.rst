...
:ref:`错误类型（Errors） <errors>` 允许您向调用者提供更多关于一个条件或操作失败原因的信息。
...
如果气体在任何时候都用完了（即会是负值），
就会触发气体不足的异常，从而恢复在当前调用帧中对状态进行的所有修改。
