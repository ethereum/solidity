...
.. 警告::
  ...
  调用将通过一个 :ref:`失败的断言 <assert-and-require>` 而被回滚。
...
合约要么有一个 :ref:`receive <receive-ether function>` 函数，要么有一个可支付的备用函数。
...
.. 注解::
    ...
    或者它的 :ref:`备用函数`，如果有的话）将与 ``transfer`` 调用一起执行（这是EVM的一个特性，无法阻止）。
...
只有在合约类型具有receive或备用支付函数的情况下，
才有可能明确转换为 ``address payable`` 类型和从该类型转换。
这种转换仍然使用 ``address(x)`` 进行转换。如果合约类型没有一个receive或备用支付函数，
