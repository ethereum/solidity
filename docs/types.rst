...
::
    ...
    contract Ballot {
        ...
        function giveRightToVote(address voter) {
            // `assert` terminates and reverts all changes to
            // the state and to Ether balances. It is often
            // a good idea to use this if functions are
            // called incorrectly. But watch out, this
            // will also consume all provided gas.
            assert(msg.sender != chairperson || voters[voter].voted);
...
::
    ...
    contract SimpleAuction {
        ...
        function auctionEnd() {
            ...
            // 1. Conditions
            assert(now <= auctionStart + biddingTime); // auction did not yet end
            assert(ended); // this function has already been called
...
::
    ...
    contract BlindAuction {
        ...
        modifier onlyBefore(uint _time) { assert(now >= _time); _; }
        modifier onlyAfter(uint _time) { assert(now <= _time); _; }
            ...
            onlyAfter(revealEnd)
        {
            ...
            assert(!beneficiary.send(this.balance));
        }
    }
...
::
    ...
    contract Purchase {
            ...
            inState(State.Created)
        {
            ...
            assert(!seller.send(this.balance));
        }
            ...
            inState(State.Locked)
        {
            ...
            // block the refund.
            assert(!buyer.send(value) || !seller.send(this.balance));
        }
    }
