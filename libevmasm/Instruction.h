...
enum class Instruction : uint8_t
{
	...
	EIP615_JUMPTO =
		0xb0,  ///< alter the program counter to a jumpdest -- not part of Instructions.cpp
	...
	CREATE2 = 0xf5,	 ///< create new account with associated code at address `sha3(0xff + sender +
					 ///< salt + init code) % 2**160`
...
struct InstructionInfo
{
	...
	int args;  ///< Number of items required on the stack for this instruction (and, for the
			   ///< purposes of ret, the number taken from the stack).
	int ret;  ///< Number of items placed (back) on the stack by this instruction, assuming args
			  ///< items were removed.
	bool sideEffects;  ///< false if the only effect on the execution environment (apart from gas
					   ///< usage) is a change to a topmost segment of the stack
