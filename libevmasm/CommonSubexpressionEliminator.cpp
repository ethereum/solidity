...
)
{
	...
	for (auto const& targetItem: m_targetStack)
	{
		...
		if (m_classPositions[targetItem.second].count(targetItem.first))
			continue;
		yul::DebugData::ConstPtr debugData;
...
void CSECodeGenerator::generateClassElement(Id _c, bool _allowSequenced)
{
	...
	yul::DebugData::ConstPtr const& itemDebugData = expr.item->debugData();
...
void CSECodeGenerator::appendDup(int _fromPosition, yul::DebugData::ConstPtr const& _debugData)
{
...
void CSECodeGenerator::appendOrRemoveSwap(int _fromPosition, yul::DebugData::ConstPtr const& _debugData)
{
	assertThrow(_fromPosition != c_invalidPosition, OptimizerException, "");
