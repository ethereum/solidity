...
)
{
	...
	u256 const mask = (u256(1) << 160) - 1;
	rules.push_back({
		{Instruction::AND, {{Instruction::CREATE, {X, Y, Z}}, mask}},
		[=]() -> Pattern { return {Instruction::CREATE, {X, Y, Z}}; },
		false
	});
	rules.push_back({
		{Instruction::AND, {{mask, {Instruction::CREATE, {X, Y, Z}}}}},
		[=]() -> Pattern { return {Instruction::CREATE, {X, Y, Z}}; },
		false
	});
...
template <class Pattern>
std::vector<SimplificationRule<Pattern>> simplificationRuleListPart10(
	Pattern,
	Pattern,
	Pattern,
	Pattern W,
...
)
{
	...
	rules += simplificationRuleListPart9(A, B, C, W, X, Y);
	rules += simplificationRuleListPart10(A, B, C, W, X, Y, Z);
	return rules;
}
