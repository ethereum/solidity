...
bool ASTJsonConverter::visit(ImportDirective const& _node)
{
	...
	attributes.emplace_back(make_pair("unitAlias", _node.name()));
	...
	attributes.emplace_back(make_pair("symbolAliases", std::move(symbolAliases)));
...
bool ASTJsonConverter::visit(VariableDeclaration const& _node)
{
	...
	if (m_inEvent)
		attributes.emplace_back(make_pair("indexed", _node.isIndexed()));
...
bool ASTJsonConverter::visit(FunctionCall const& _node)
{
	...
	if (m_legacy)
	{
		attributes.emplace_back(make_pair("isStructConstructorCall", _node.annotation().kind == FunctionCallKind::StructConstructorCall));
		attributes.emplace_back(make_pair("type_conversion", _node.annotation().kind == FunctionCallKind::TypeConversion));
	}
	else
		attributes.emplace_back(make_pair("kind", functionCallKind(_node.annotation().kind)));
	appendExpressionAttributes(attributes, _node.annotation());
