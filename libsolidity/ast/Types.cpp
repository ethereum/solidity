...
MemberList::MemberMap AddressType::nativeMembers(ContractDefinition const *) const {
    MemberList::MemberMap members = {
            ...
            {"call",         TypeProvider::function(strings{"bytes memory"}, strings{"bool", "bytes memory"},
                                                    FunctionType::Kind::BareCall, false, StateMutability::Payable)},
            {"callcode",     TypeProvider::function(strings{"bytes memory"}, strings{"bool", "bytes memory"},
                                                    FunctionType::Kind::BareCallCode, false, StateMutability::Payable)},
            {"delegatecall", TypeProvider::function(strings{"bytes memory"}, strings{"bool", "bytes memory"},
                                                    FunctionType::Kind::BareDelegateCall, false,
                                                    StateMutability::NonPayable)},
            {"staticcall",   TypeProvider::function(strings{"bytes memory"}, strings{"bool", "bytes memory"},
                                                    FunctionType::Kind::BareStaticCall, false, StateMutability::View)},
            {"tokenBalance", TypeProvider::function(strings{"grcToken"}, strings{"uint"},
                                                    FunctionType::Kind::TokenBalance, false, StateMutability::View)},
    };
    if (m_stateMutability == StateMutability::Payable) {
        members.emplace_back(MemberList::Member{"send", TypeProvider::function(strings{"uint"}, strings{"bool"},
                                                                               FunctionType::Kind::Send, false,
                                                                               StateMutability::NonPayable)});
        members.emplace_back(MemberList::Member{"transfer", TypeProvider::function(strings{"uint"}, strings(),
                                                                                   FunctionType::Kind::Transfer, false,
                                                                                   StateMutability::NonPayable)});
        members.emplace_back(MemberList::Member{"transferToken",
                                                TypeProvider::function(strings{"uint", "grcToken"}, strings(),
...
TypeResult IntegerType::unaryOperatorResult(Token _operator) const {
    ...
    if (_operator == Token::Delete)
        ...
        // no further unary operators for grcToken
    else if (isGrcToken())
        return TypeResult::err("");
        // for non-grctoken integers
        // we allow -, ++ and --
...
IntegerType const *FixedPointType::asIntegerType() const {
    return TypeProvider::integer(numBits(),
...
TypeResult RationalNumberType::binaryOperatorResult(Token _operator, Type const *_other) const {
    ...
    if (TokenTraits::isCompareOp(_operator)) {
                ...
                if (m_value.numerator() == 0)
                    ...
                    if (exponent > mostSignificantBit(boost::multiprecision::abs(m_value.numerator())))
                        ...
                        if (m_value.numerator() < 0)
                            ...
                            value = rational(
                                    (m_value.numerator() + 1) / boost::multiprecision::pow(bigint(2), exponent) -
                                    bigint(1), 1);
                        else
        ...
        if (value.numerator() != 0 &&
            max(mostSignificantBit(abs(value.numerator())), mostSignificantBit(abs(value.denominator()))) > 4096)
            return TypeResult::err("Precision of rational constants is limited to 4096 bits.");
...
FunctionTypePointer FunctionType::interfaceFunctionType() const {
    ...
    bool isLibraryFunction =
            kind() != Kind::Event && dynamic_cast<ContractDefinition const &>(*m_declaration->scope()).isLibrary();
    ...
    switch (m_kind) {
        case Kind::Block:
            return MemberList::MemberMap({
                                                 ...
                                                 {"blockhash",  TypeProvider::function(strings{"uint"},
                                                                                       strings{"bytes32"},
