		...
		object "<CreationObject>" {
			...
			object "<DeployedObject>" {
				code {
					...
					<?library>
						<?eof>
							let called_via_delegatecall := iszero(eq(auxdataloadn(<library_address_offset>), address()))
						<!eof>
		...
		if (_varDecl.immutable())
		{
			...
			auto t = Whiskers(R"(
				...
				function <functionName>() -> rval {
					<?eof>
						rval := auxdataloadn(<immutableDataSectionOffset>)
					<!eof>
	...
	if (_contract.isLibrary())
	{
		...
		else
		{
			...
			immutables.emplace_back(std::map<std::string, std::string>{
				...
				{"immutablesOffset"s, std::to_string(m_context.immutableMemoryOffset(*(*varIt)))},
				{"value"s, "address()"}
			});
		}
	}
