...
{
	string name = buildFunctionName(_function);
...
string IRGenerationContext::internalDispatch(Arity const& _arity, set<FunctionDefinition const*> const& _functions)
{
	...
	{
		solAssert(function, "");
		solAssert(getFunctionArity(*function) == _arity, "A single dispatch function can only handle functions of one arity");
		...
		cases.emplace_back(map<string, string>{
			...
			{"name", buildFunctionName(*function)}
		});
	}

	string funName = buildInternalDispatchFunctionName(_arity);
