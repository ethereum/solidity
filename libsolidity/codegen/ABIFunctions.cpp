	...
	return createFunction(
		...
		[&]()
		{
			...
			for (size_t i = 0; i < _givenTypes.size(); ++i)
			{
				...
				elementTempl(
					"abiEncode",
					abiEncodingFunction(*_givenTypes[i], *_targetTypes[i], options)
				);
			...
			string valueParams = _reversed ? suffixedVariableNameList("value", stackPos, 0) :
		...
		[&]()
		{
			...
			for (size_t i = 0; i < _givenTypes.size(); ++i)
			{
				...
				if (!dynamic)
					elementTempl(
						"calldataEncodedSize",
						to_string(_targetTypes[i]->calldataEncodedSize(false))
					);
				elementTempl(
					"abiEncode",
					abiEncodingFunction(*_givenTypes[i], *_targetTypes[i], options)
	...
	return createFunction(
		...
		[&]()
		{
			...
			for (size_t i = 0; i < _types.size(); ++i)
			{
				...
				elementTempl(
					"revertString",
					revertReasonIfDebug("ABI decoding: invalid tuple offset")
				);
...
)
{
	...
	else if (auto toArray = dynamic_cast<ArrayType const*>(&to))
	{
		...
		case DataLocation::CallData:
			if (fromArray->isByteArray() || *fromArray->baseType() == *TypeProvider::uint256() ||
				*fromArray->baseType() == FixedBytesType(32))
				return abiEncodingFunctionCalldataArrayWithoutCleanup(
					*fromArray,
					*toArray,
					_options
				);
	...
	string functionName = "abi_encode_" + _from.identifier() + "_to_" + to.identifier() +
	...
	return createFunction(
		...
		[&]()
		{
			...
			if (_from.dataStoredIn(DataLocation::Storage))
			{
				...
				solAssert(
					_options.padded && !_options.dynamicInplace,
					"Non-padded / inplace encoding for library call requested."
				);
...
{
	string functionName = "abi_encodeUpdatedPos_" + _givenType.identifier() + "_to_" +
		_targetType.identifier() + _options.toFunctionNameSuffix();
	return createFunction(
		functionName,
		[&]()
		{
			string values =
				suffixedVariableNameList("value", 0, numVariablesForType(_givenType, _options));
			...
			)")("functionName", functionName)("encode", encoder)("values", values)
					.render();
			else
			{
			...
			)")("functionName", functionName)("encode", encoder)(
						   "encodedSize",
						   toCompactHexWithPrefix(encodedSize)
				)("values", values)
					.render();
			}
		}
...
)
{
	...
	solAssert(
		fromArrayType.isByteArray() || *fromArrayType.baseType() == *TypeProvider::uint256() ||
	...
	string functionName = "abi_encode_" + _from.identifier() + "_to_" + _to.identifier() +
		_options.toFunctionNameSuffix();
	return createFunction(
		functionName,
		[&]()
		{
			...
			if (fromArrayType.isDynamicallySized())
			{
				...
				if (fromArrayType.isByteArray() || fromArrayType.calldataStride() == 1)
					...
					templ(
						...
						length := mul(length, <stride>)
							...
							"maxLength",
							toCompactHexWithPrefix(u256(-1) / fromArrayType.calldataStride())
				...
				templ(
					"lengthPadded",
					needsPadding ? m_utils.roundUpFunction() + "(length)" : "length"
				);
				return templ.render();
			}
			else
			{
				...
				templ(
					"byteLength",
					toCompactHexWithPrefix(fromArrayType.length() * fromArrayType.calldataStride())
				);
				return templ.render();
			}
		}
...
)
{
	...
	return createFunction(
		...
		[&]()
		{
			...
			string elementValues = suffixedVariableNameList(
				"elementValue",
				0,
				numVariablesForType(*_from.baseType(), subOptions)
			);
			Whiskers templ(
				usesTail ?
					  R"(
			...
			bool lengthAsArgument =
				_from.dataStoredIn(DataLocation::CallData) && _from.isDynamicallySized();
			if (lengthAsArgument)
			{
			...
			else
			{
				...
				templ(
					"declareLength",
					"let length := " + m_utils.arrayLengthFunction(_from) + "(value)"
				);
			}
			templ("readableTypeNameFrom", _from.toString(true));
			templ("readableTypeNameTo", _to.toString(true));
			...
			templ(
				"encodeToMemoryFun",
				abiEncodeAndReturnUpdatedPosFunction(*_from.baseType(), *_to.baseType(), subOptions)
			);
			...
			case DataLocation::Storage:
				if (_from.baseType()->isValueType())
					templ(
						"arrayElementAccess",
						m_utils.readFromStorage(*_from.baseType(), 0, false) + "(srcPtr)"
					);
				else
			...
			case DataLocation::CallData:
				templ(
					"arrayElementAccess",
					calldataAccessFunction(*_from.baseType()) + "(baseRef, srcPtr)"
				);
				break;
			default:
				solAssert(false, "");
...
)
{
	...
	return createFunction(
		...
		[&]()
		{
			...
			templ(
				"lengthPadded",
				_options.padded ? m_utils.roundUpFunction() + "(length)" : "length"
			);
			return templ.render();
		}
	);
	...
	return createFunction(
		...
		[&]()
		{
			...
			else
			{
				...
				subOptions.encodeFunctionFromStack = false;
				subOptions.padded = true;
				string encodeToMemoryFun =
					abiEncodingFunction(*_from.baseType(), *_to.baseType(), subOptions);
				...
				for (size_t i = 0; i < itemsPerSlot; ++i)
				{
					if (_from.isDynamicallySized())
						...
						items[i]["inRange"] = "0";
					items[i]["extractFromSlot"] =
						m_utils.extractFromStorageValue(*_from.baseType(), i * storageBytes, false);
				}
				templ("items", items);
				return templ.render();
			}
	...
	return createFunction(
		...
		[&]()
		{
			...
			for (auto const& member: _to.members(nullptr))
			{
				...
				TypePointer memberTypeTo =
					member.type->fullEncodingType(_options.encodeAsLibraryTypes, true, false);
				solUnimplementedAssert(
					memberTypeTo,
				...
				case DataLocation::Storage:
				{
					...
					if (memberTypeFrom->isValueType())
					{
						...
						{
							members.back()["preprocess"] = "slotValue := sload(add(value, " +
								toCompactHexWithPrefix(storageSlotOffset) + "))";
							previousSlotOffset = storageSlotOffset;
						}
						members.back()["retrieveValue"] = m_utils.extractFromStorageValue(
															  *memberTypeFrom,
															  intraSlotOffset,
															  false
														  ) +
					...
					else
					{
						...
						members.back()["retrieveValue"] =
							"add(value, " + toCompactHexWithPrefix(storageSlotOffset) + ")";
					}
					break;
				}
				case DataLocation::Memory:
				...
				{
					string sourceOffset =
						toCompactHexWithPrefix(_from.calldataOffsetOfMember(member.name));
					members.back()["retrieveValue"] = calldataAccessFunction(*memberTypeFrom) +
						"(value, add(value, " + sourceOffset + "))";
					break;
				}
				default:
					solAssert(false, "");
				...
				string memberValues = suffixedVariableNameList(
					"memberValue",
					0,
					numVariablesForType(*memberTypeFrom, subOptions)
				);
				...
				else
				{
					...
					encodeTempl(
						"abiEncode",
						abiEncodingFunction(*memberTypeFrom, *memberTypeTo, subOptions)
					);
	...
	return createFunction(
		...
		[&]()
		{
			...
			if (_to.isDynamicallySized())
			{
				...
				templ(
					"storeLength",
					arrayStoreLengthForEncodingFunction(
...
)
{
	...
	if (_options.encodeFunctionFromStack)
		return createFunction(
			...
			[&]()
			{
				...
				function <functionName>(addr, function_id, pos) {
					...
					functionName)("combineExtFun", m_utils.combineExternalFunctionIdFunction())
					.render();
			}
		);
	else
		return createFunction(
			...
			)")("functionName", functionName)("cleanExtFun", m_utils.cleanupFunction(_to))
					.render();
			}
		);
}
...
string ABIFunctions::abiDecodingFunctionValueType(Type const& _type, bool _fromMemory)
{
	...
	string functionName = "abi_decode_" + _type.identifier() + (_fromMemory ? "_fromMemory" : "");
	return createFunction(
		functionName,
		[&]()
		{
...
string ABIFunctions::abiDecodingFunctionArray(ArrayType const& _type, bool _fromMemory)
{
	...
	return createFunction(
		...
		[&]()
		{
			...
			templ(
				"revertString",
				revertReasonIfDebug("ABI decoding: invalid calldata array offset")
			);
			templ("functionName", functionName);
			templ("readableTypeName", _type.toString(true));
			templ(
				"retrieveLength",
				!_type.isDynamicallySized() ? toCompactHexWithPrefix(_type.length()) :
												load + "(offset)"
			);
			...
			if (_type.isDynamicallySized())
				templ(
					"storeLength",
					"mstore(array, length) offset := add(offset, 0x20) dst := add(dst, 0x20)"
				);
			else
				templ("storeLength", "");
			if (dynamicBase)
			...
			else
			{
				templ(
					"staticBoundsCheck",
					"if gt(add(src, mul(length, " + calldataStride + ")), end) { " +
...
string ABIFunctions::abiDecodingFunctionCalldataArray(ArrayType const& _type)
{
	...
	return createFunction(
		...
		[&]()
		{
			...
			Whiskers w{templ};
			// TODO add test
			w("revertStringOffset",
			  revertReasonIfDebug("ABI decoding: invalid calldata array offset"));
			w("revertStringLength",
			  revertReasonIfDebug("ABI decoding: invalid calldata array length"));
			w("revertStringPos",
			  revertReasonIfDebug("ABI decoding: invalid calldata array stride"));
...
string ABIFunctions::abiDecodingFunctionByteArray(ArrayType const& _type, bool _fromMemory)
{
	...
	return createFunction(
		...
		[&]()
		{
			...
			templ(
				"revertStringOffset",
				revertReasonIfDebug("ABI decoding: invalid byte array offset")
			);
			templ(
				"revertStringLength",
				revertReasonIfDebug("ABI decoding: invalid byte array length")
			);
			templ("functionName", functionName);
			templ("load", _fromMemory ? "mload" : "calldataload");
			templ("allocate", m_utils.allocationFunction());
...
string ABIFunctions::abiDecodingFunctionCalldataStruct(StructType const& _type)
{
	...
	return createFunction(
		...
		[&]()
		{
			...
			w("minimumSize",
			  to_string(
				  _type.isDynamicallyEncoded() ? _type.calldataEncodedTailSize() :
												   _type.calldataEncodedSize(true)
			  ));
			return w.render();
		}
	);
	...
	return createFunction(
		...
		[&]()
		{
			...
			for (auto const& member: _type.members(nullptr))
			{
				...
				memberTempl(
					"revertString",
					revertReasonIfDebug("ABI decoding: invalid struct offset")
				);
				memberTempl("load", _fromMemory ? "mload" : "calldataload");
				memberTempl("pos", to_string(headPos));
				memberTempl(
					"memoryOffset",
					toCompactHexWithPrefix(_type.memoryOffsetOfMember(member.name))
				);
...
string ABIFunctions::abiDecodingFunctionFunctionType(
...
)
{
	...
	return createFunction(
		...
		[&]()
		{
			if (_forUseOnStack)
			{
				...
				function <functionName>(offset, end) -> addr, function_selector {
					addr, function_selector := <splitExtFun>(<decodeFun>(offset, end))
						   ...
						   "decodeFun",
						   abiDecodingFunctionFunctionType(_type, _fromMemory, false)
			...
			)")("functionName", functionName)("load", _fromMemory ? "mload" : "calldataload")(
						   "validateExtFun",
						   m_utils.validatorFunction(_type, true)
				)
					.render();
			}
		}
	...
	return createFunction(
		...
		[&]()
		{
			...
			else if (_type.isValueType())
			{
				...
				// Note that the second argument to the decoding function should be discarded after
				// inlining.
				return Whiskers(R"(
				function <functionName>(baseRef, ptr) -> value {
					value := <decodingFunction>(ptr, add(ptr, 32))
				}
			)")("functionName", functionName)("decodingFunction", decodingFunction)
					.render();
			}
			else
			{
...
string ABIFunctions::arrayStoreLengthForEncodingFunction(
	ArrayType const& _type,
	EncodingOptions const& _options
)
{
	string functionName =
		"array_storeLengthForEncoding_" + _type.identifier() + _options.toFunctionNameSuffix();
	return createFunction(
		functionName,
		[&]()
		{
