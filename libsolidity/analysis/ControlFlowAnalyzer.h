...
private:
	/// Checks for uninitialized variable accesses in the control flow between @param _entry and
	/// @param _exit.
	void checkUninitializedAccess(CFGNode const* _entry, CFGNode const* _exit) const;
	/// Checks for unreachable code, i.e. code ending in @param _exit, @param _revert or @param
	/// _transactionReturn that can not be reached from @param _entry.
