...
bool ReferencesResolver::visit(InlineAssembly const& _inlineAssembly)
{
	...
	m_nameRepository = &_inlineAssembly.operations().nameRepository();
	(*this)(_inlineAssembly.operations().block());
	m_nameRepository = nullptr;
...
{
	solAssert(m_nameRepository);
	solAssert(nativeLocationOf(_function) == originLocationOf(_function), "");
	validateYulIdentifierName(m_nameRepository->requiredLabelOf(_function.name), nativeLocationOf(_function));
	for (yul::TypedName const& varName: _function.parameters + _function.returnVariables)
	{
		...
		validateYulIdentifierName(m_nameRepository->requiredLabelOf(varName.name), nativeLocationOf(varName));
	}
...
void ReferencesResolver::operator()(yul::Identifier const& _identifier)
{
	...
	auto const identifierLabel = m_nameRepository->requiredLabelOf(_identifier.name);
	if (m_resolver.experimentalSolidity())
	{
...
void ReferencesResolver::operator()(yul::VariableDeclaration const& _varDecl)
{
	...
	for (auto const& identifier: _varDecl.variables)
	{
		...
		validateYulIdentifierName(m_nameRepository->requiredLabelOf(identifier.name), nativeLocationOf(identifier));

		if (
			auto declarations = m_resolver.nameFromCurrentScope(std::string(m_nameRepository->requiredLabelOf(identifier.name)));
			!declarations.empty()
		)
		{
