...
bool SyntaxChecker::visit(PragmaDirective const& _pragma)
{
	...
	if (_pragma.tokens()[0] != Token::Identifier)
		m_errorReporter.syntaxError(
			5226_error,
			_pragma.location(),
			"Invalid pragma \"" + _pragma.literals()[0] + "\""
		);
	else if (_pragma.literals()[0] == "experimental")
	{
		...
		if (literals.empty())
			...
			m_errorReporter.syntaxError(6022_error, _pragma.location(), "Stray arguments.");
		else
		{
			...
			else
			{
				...
				if (!ExperimentalFeatureWithoutWarning.count(feature))
					m_errorReporter.warning(
						2264_error,
						_pragma.location(),
						"Experimental features are turned on. Do not use experimental features on "
						"live deployments."
					);
			}
		}
	...
	else if (_pragma.literals()[0] == "solidity")
	{
		...
		m_errorReporter.syntaxError(
			4936_error,
			_pragma.location(),
			"Unknown pragma \"" + _pragma.literals()[0] + "\""
		);
	return true;
}
...
bool SyntaxChecker::visit(Literal const& _literal)
{
	if ((_literal.token() == Token::UnicodeStringLiteral) && !validateUTF8(_literal.value()))
		m_errorReporter
			.syntaxError(8452_error, _literal.location(), "Invalid UTF-8 sequence found");
	...
	if (value.find("__") != ASTString::npos)
	{
		m_errorReporter.syntaxError(
			2990_error,
			_literal.location(),
			"Invalid use of underscores in number literal. Only one consecutive underscores "
	...
	if (!_literal.isHexNumber())  // decimal literal
	{
		if (value.find("._") != ASTString::npos)
			m_errorReporter.syntaxError(
				3891_error,
				_literal.location(),
				"Invalid use of underscores in number literal. No underscores in front of the "
				"fraction part allowed."
			);
		...
		if (value.find("_e") != ASTString::npos)
			m_errorReporter.syntaxError(
				6415_error,
				_literal.location(),
				"Invalid use of underscores in number literal. No underscore at the end of the "
				"mantissa allowed."
			);
		...
		if (value.find("e_") != ASTString::npos)
			m_errorReporter.syntaxError(
				6165_error,
				_literal.location(),
				"Invalid use of underscores in number literal. No underscore in front of exponent "
				"allowed."
			);
	}
...
bool SyntaxChecker::visit(UnaryOperation const& _operation)
{
	if (_operation.getOperator() == Token::Add)
		m_errorReporter
			.syntaxError(9636_error, _operation.location(), "Use of unary + is disallowed.");
...
bool SyntaxChecker::visit(InlineAssembly const& _inlineAssembly)
{
	...
	if (yul::MSizeFinder::containsMSize(_inlineAssembly.dialect(), _inlineAssembly.operations()))
		m_errorReporter.syntaxError(
			...
			"it can change its semantics. Either disable the Yul optimizer or do not use the "
			"instruction."
		);
	...
	for (FunctionDefinition const* function: _contract.definedFunctions())
		if (function->name() == contractName)
			m_errorReporter.syntaxError(
				...
				"If you intend this to be a constructor, use \"constructor(...) { ... }\" to "
				"define it."
			);
	return true;
}
...
bool SyntaxChecker::visit(FunctionDefinition const& _function)
{
	if (!_function.isConstructor() && _function.noVisibilitySpecified())
	{
		string suggestedVisibility =
			_function.isFallback() || _function.isReceive() || m_isInterface ? "external" :
																				 "public";
		m_errorReporter.syntaxError(
			4937_error,
			_function.location(),
...
{
	// Report if none of the variable components in the tuple have a name (only possible via
	// deprecated "var")
	if (std::all_of(
			_statement.declarations().begin(),
			_statement.declarations().end(),
			[](ASTPointer<VariableDeclaration> const& declaration)
			{ return declaration == nullptr; }
		))
		m_errorReporter.syntaxError(
			3299_error,
			_statement.location(),
			"The use of the \"var\" keyword is disallowed. The declaration part of the statement "
			"can be removed, since it is empty."
		);
...
bool SyntaxChecker::visit(StructDefinition const& _struct)
{
	if (_struct.members().empty())
		m_errorReporter
			.syntaxError(5306_error, _struct.location(), "Defining empty structs is disallowed.");
