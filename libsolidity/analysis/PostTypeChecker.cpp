...
{
public:
	YulLValueChecker(ASTString const& _identifierName, yul::YulNameRepository const& _nameRepository):
		m_identifierName(_identifierName), m_nameRepository(_nameRepository) {}
	...
	void operator()(solidity::yul::Assignment const& _assignment) override
	{
		...
		if (ranges::any_of(
			...
			[&](auto const& yulIdentifier) { return m_nameRepository.requiredLabelOf(yulIdentifier.name) == m_identifierName; }
		))
			m_willBeWrittenTo = true;
	}
private:
	ASTString const& m_identifierName;
	yul::YulNameRepository const& m_nameRepository;
