...
function balance(x1, x2, x3, x4) -> z1, z2, z3, z4 {
	// address x is a big endian encoded unsigned integer.
	// mstore_address(pos, address) will write that address as a little endian
	// unsigned integer at offset 0.
	mstore_address(0:i32, x1, x2, x3, x4)

	// getExternalBalance(addressOffset, resultOffset): Gets balance of the
	// given account and loads it into memory at the given offset.
...
// call data starting from position x (32 bytes)
...
// copy z bytes from calldata at position y to mem at position x
...
// copy z bytes from code at position y to mem at position x
...
// mload_internal(p): bswap(mem[p…(p+32)))
...
// mload_address(pos:i32): loads address from pos
...
// mstore_address(p, a): writes bswap(address) a to pos
...
// create(v, p, n): create new contract with code mem[p…(p+n)) and send v wei
...
// call(g, a, v, in, insize, out, outsize): call contract at address a with
//     input mem[in…(in+insize)) providing g gas and v wei and output area
//     mem[out…(out+outsize)) returning 0 on error (eg. out of gas) and 1 on success.
function call(
	a1, a2, a3, a4,
	b1, b2, b3, b4,
	c1, c2, c3, c4,
...
// callcode(g, a, v, in, insize, out, outsize): identical to call but only use
//     the code from a and stay in the context of the current contract otherwise.
function callcode(
	a1, a2, a3, a4,
	b1, b2, b3, b4,
	c1, c2, c3, c4,
...
// delegatecall(g, a, in, insize, out, outsize): identical to callcode
//     but also keep caller and callvalue.
...
// staticcall(g, a, in, insize, out, outsize): identical to
//     call(g, a, 0, in, insize, out, outsize) but do not allow state modifications.
function staticcall(
	a1, a2, a3, a4,
	b1, b2, b3, b4,
	c1, c2, c3, c4,
...
// create2(v, p, n, s): create new contract with code mem[p…(p+n)) at
