...
{
	if (m_yulNameRepository.isBuiltinName(_originalName) || m_externallyUsedIdentifiers.count(_originalName))
	...
	if (!m_translations.count(id))
		m_translations[id] = m_yulNameRepository.deriveName(_originalName);
