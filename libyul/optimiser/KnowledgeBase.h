...
public:
	...
	explicit KnowledgeBase(std::function<AssignedValue const*(YulName)> _variableValues, YulNameRepository const& _yulNameRepository):
		m_variableValues(std::move(_variableValues)), m_yulNameRepository(_yulNameRepository)
	{}
	/// Constructor to use if source code is in SSA form and values are constant.
	explicit KnowledgeBase(std::map<YulName, AssignedValue> const& _ssaValues, YulNameRepository const& _yulNameRepository);
