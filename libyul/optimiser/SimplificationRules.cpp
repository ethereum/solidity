...
bool Pattern::matches(Expression const& _expr, map<YulString, Expression const*> const& _ssaValues) const
{
	...
	if (m_kind == PatternKind::Constant)
	{
		...
		if (literal.kind != yul::LiteralKind::Number)
			return false;
...
Expression Pattern::toExpression(SourceLocation const& _location) const
{
	...
	if (m_kind == PatternKind::Constant)
	{
		...
		return Literal{_location, yul::LiteralKind::Number, YulString{formatNumber(*m_data)}, {}};
	}
	else if (m_kind == PatternKind::Operation)
	{
...
u256 Pattern::d() const
{
	...
	assertThrow(literal.kind == yul::LiteralKind::Number, OptimizerException, "");
