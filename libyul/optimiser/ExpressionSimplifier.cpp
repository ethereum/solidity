...
{
	ExpressionSimplifier{_context.yulNameRepository}(_ast);
}
...
void ExpressionSimplifier::visit(Expression& _expression)
{
	...
	while (auto const* match = SimplificationRules::findFirstMatch(
		...
		_expression = match->action().toExpression(debugDataOf(_expression), evmVersionFromDialect(m_yulNameRepository.dialect()), m_yulNameRepository);

	if (auto* functionCall = std::get_if<FunctionCall>(&_expression))
		if (std::optional<evmasm::Instruction> instruction = toEVMInstruction(m_yulNameRepository, functionCall->functionName.name))
