...
void VarDeclInitializer::operator()(Block& _block)
{
	...
	util::GenericVisitor visitor{
		...
		[this](VariableDeclaration& _varDecl) -> OptionalStatements
		{
			...
			{
				_varDecl.value = std::make_unique<Expression>(m_nameRepository.dialect().zeroLiteralForType(_varDecl.variables.front().type));
				return {};
			}
			else
			{
				...
				{
					std::unique_ptr<Expression> expr = std::make_unique<Expression >(m_nameRepository.dialect().zeroLiteralForType(var.type));
