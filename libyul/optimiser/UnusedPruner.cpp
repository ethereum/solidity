	...
	for (auto&& statement: _block.statements)
		...
		else if (holds_alternative<VariableDeclaration>(statement))
		{
			...
			))
			{
				...
				else if (varDecl.variables.size() == 1 && m_dialect.discardFunction(varDecl.variables[0].type))
					statement = ExpressionStatement{varDecl.location, FunctionCall{
						varDecl.location,
						{varDecl.location, m_dialect.discardFunction(varDecl.variables[0].type)->name},
