...
KnowledgeBase::KnowledgeBase(std::map<YulName, AssignedValue> const& _ssaValues, YulNameRepository const& _yulNameRepository):
	m_valuesAreSSA(true),
...
std::optional<KnowledgeBase::VariableOffset> KnowledgeBase::explore(Expression const& _value)
{
	...
	{
		if (f->functionName.name == m_yulNameRepository.predefined().add)
		{
		...
		else if (f->functionName.name == m_yulNameRepository.predefined().sub)
			if (std::optional<VariableOffset> a = explore(f->arguments[0]))
				if (std::optional<VariableOffset> b = explore(f->arguments[1]))
				{
					u256 offset = a->offset - b->offset;
