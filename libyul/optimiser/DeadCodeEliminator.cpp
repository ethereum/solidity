...
{
	ControlFlowSideEffectsCollector sideEffects(_context.nameRepository, _ast);
	DeadCodeEliminator{
...
void DeadCodeEliminator::operator()(Block& _block)
{
	...
	size_t index;
	std::tie(controlFlowChange, index) = TerminationFinder{m_nameRepository, &m_functionSideEffects}.firstUnconditionalControlFlowChange(_block.statements);
