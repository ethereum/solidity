...
GRAMMAR_FILE="$(readlink -f "${ROOT_DIR}/tools/Solidity.g4")"
...
if [[ ! -f "${WORKDIR}/target/SolidityParser.class" ]]; then
...
test_file() {
    ...
    if [[ "${output}" == "" ]]; then
        ...
        #exit 1 # XXX DON"T abort early, now now.
    fi
}

SOL_FILES=( $(find "${ROOT_DIR}/test/libsolidity/syntaxTests" -name '*.sol') )
