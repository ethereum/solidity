...
public:
	...
	using mutable_value_type = typename std::
		conditional<std::is_const<T>::value, typename std::remove_const<T>::type, T>::type;
	using string_type =
		typename std::conditional<std::is_const<T>::value, std::string const, std::string>::type;
	using vector_type = typename std::conditional<
		std::is_const<T>::value,
		std::vector<typename std::remove_const<T>::type> const,
		std::vector<T>>::type;
	using iterator = T*;
	...
	std::vector<unsigned char> toBytes() const
	{
		return std::vector<unsigned char>(
			...
			reinterpret_cast<unsigned char const*>(m_data) + m_count * sizeof(T)
		);
	}
	std::string toString() const
	{
		return std::string((char const*) m_data, ((char const*) m_data) + m_count * sizeof(T));
	}
	...
	bool operator==(vector_ref<T> const& _cmp) const
	{
