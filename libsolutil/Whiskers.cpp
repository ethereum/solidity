...
Whiskers::Whiskers(string _template): m_template(move(_template)) {}
...
Whiskers& Whiskers::operator()(string _listParameter, vector<map<string, string>> _values)
{
			...
			else
			{
				...
				if (conditionName[0] == '+')
				{
					...
					assertThrow(
						...
						WhiskersError,
						"Tag " + tag + " used as condition but was not set."
					);
				...
				else
				{
					assertThrow(
						...
						WhiskersError,
						"Condition parameter " + conditionName + " not set."
					);
...
Whiskers::StringMap Whiskers::joinMaps(Whiskers::StringMap const& _a, Whiskers::StringMap const& _b)
{
