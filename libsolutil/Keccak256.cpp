...
/*** Constants. ***/
static uint8_t const rho[24] = {1,	3,	6,	10, 15, 21, 28, 36, 45, 55, 2,	14,
								27, 41, 56, 8,	25, 43, 62, 18, 39, 61, 20, 44};
static uint8_t const pi[24] = {10, 7,  11, 17, 18, 3, 5,  16, 8,  21, 24, 4,
...
static inline void keccakf(void* state)
{
	...
	for (int i = 0; i < 24; i++)
	{
		uint8_t x, y;
		// Theta
		FOR5(uint8_t, x, 1, b[x] = 0; FOR5(uint8_t, y, 5, b[x] ^= a[x + y];))
		FOR5(
			uint8_t,
			x,
			1,
			FOR5(uint8_t, y, 5, a[y + x] ^= b[(x + 4) % 5] ^ rol(b[(x + 1) % 5], 1);)
		)
		// Rho and pi
		uint64_t t = a[1];
		x = 0;
		...
		FOR5(
			...
			y,
			5,
			FOR5(uint8_t, x, 1, b[x] = a[y + x];)
				FOR5(uint8_t, x, 1, a[y + x] = b[x] ^ ((~b[(x + 1) % 5]) & b[(x + 2) % 5]);)
		)
		// Iota
		a[0] ^= RC[i];
	}
...
#define mkapply_ds(NAME, S) \
	static inline void NAME(uint8_t* dst, uint8_t const* src, size_t len) { FOR(i, 1, len, S); }
#define mkapply_sd(NAME, S) \
	static inline void NAME(uint8_t const* src, uint8_t* dst, size_t len) { FOR(i, 1, len, S); }
