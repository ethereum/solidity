...
template <class T> inline std::vector<T> operator+(std::vector<T> const &_a, std::vector<T> const &_b)
{
	std::vector<T> ret(_a);
	ret += _b;
	return ret;
...
template <class T> inline std::vector<T> operator+(std::vector<T> &&_a, std::vector<T> &&_b)
{
...
template <class T, class U> inline std::set<T> operator+(std::set<T> const &_a, U &&_b)
{
	std::set<T> ret(_a);
...
template <class T, class U> inline std::set<T> operator+(std::set<T> &&_a, U &&_b)
{
...
template <class T> inline std::set<T> &operator-=(std::set<T> &_a, std::set<T> const &_b)
{
	for (auto const &x : _b)
		_a.erase(x);
	return _a;
...
inline bytes toCompactBigEndian(uint8_t _val, unsigned _min = 0) { return (_min || _val) ? bytes{_val} : bytes{}; }
...
template <typename T, typename F> void iterateReplacing(std::vector<T> &_vector, F const &_f)
{
...
/// place at the end, but already visited elements might be invalidated.
/// If nothing is replaced, no copy is performed.
template <std::size_t N, typename T, typename F> void iterateReplacingWindow(std::vector<T> &_vector, F const &_f)
{
...
{
	return std::equal(std::begin(_lhs),
	                  std::end(_lhs),
	                  std::begin(_rhs),
	                  std::end(_rhs),
...
namespace detail
{
template <typename T> void variadicEmplaceBack(std::vector<T> &) {}
...
template <typename T, typename... Args> std::vector<T> make_vector(Args &&... _args)
{
	std::vector<T> result;
