...
BOOST_AUTO_TEST_CASE(empty_code) { BOOST_CHECK_EQUAL(codeSize("{}"), 0); }
...
BOOST_AUTO_TEST_CASE(nested_blocks) { BOOST_CHECK_EQUAL(codeSize("{ {} {} {{ }} }"), 0); }
...
BOOST_AUTO_TEST_CASE(instruction) { BOOST_CHECK_EQUAL(codeSize("{ pop(calldatasize()) }"), 2); }

BOOST_FIXTURE_TEST_CASE(instruction_custom_weights, CustomWeightFixture)
{
	BOOST_CHECK_EQUAL(
		codeSize("{ pop(calldatasize()) }", m_weights),
		2 * m_weights.functionCallCost + 1 * m_weights.expressionStatementCost
	);
}
...
BOOST_AUTO_TEST_CASE(constants_cost_one) { BOOST_CHECK_EQUAL(codeSize("{ let x := 3 }"), 1); }

BOOST_FIXTURE_TEST_CASE(constants_custom_weights, CustomWeightFixture)
{
	BOOST_CHECK_EQUAL(
		codeSize("{ let x := 3 }", m_weights),
		1 * m_weights.variableDeclarationCost + 1 * m_weights.literalCost
	);
}
...
BOOST_FIXTURE_TEST_CASE(function_with_arguments_custom_weights, CustomWeightFixture)
{
	BOOST_CHECK_EQUAL(
		...
		1 * m_weights.expressionStatementCost + 1 * m_weights.functionCallCost +
...
BOOST_FIXTURE_TEST_CASE(function_with_variables_as_arguments_custom_weights, CustomWeightFixture)
{
	BOOST_CHECK_EQUAL(
		...
		1 * m_weights.variableDeclarationCost + 1 * m_weights.expressionStatementCost +
			1 * m_weights.functionCallCost + 1 * m_weights.identifierCost
	);
}
...
{
	BOOST_CHECK_EQUAL(
		codeSize("{ function f(x, r) -> z { sstore(x, r) z := r } let y let t := f(y, 2) }"),
		2
	);
}
...
)
{
	BOOST_CHECK_EQUAL(
		...
		2 * m_weights.variableDeclarationCost + 1 * m_weights.functionCallCost +
			1 * m_weights.identifierCost + 1 * m_weights.literalCost
	);
}

BOOST_AUTO_TEST_CASE(assignment) { BOOST_CHECK_EQUAL(codeSize("{ let a a := 3 }"), 1); }

BOOST_FIXTURE_TEST_CASE(assignment_custom_weights, CustomWeightFixture)
{
	BOOST_CHECK_EQUAL(
		codeSize("{ let a a := 3 }", m_weights),
		1 * m_weights.variableDeclarationCost + 1 * m_weights.assignmentCost +
			1 * m_weights.literalCost
	);
}
...
BOOST_FIXTURE_TEST_CASE(assignments_between_vars_are_free_custom_weights, CustomWeightFixture)
{
	BOOST_CHECK_EQUAL(
		...
		2 * m_weights.variableDeclarationCost + 1 * m_weights.assignmentCost +
...
BOOST_FIXTURE_TEST_CASE(assignment_complex_custom_weights, CustomWeightFixture)
{
	BOOST_CHECK_EQUAL(
		...
		2 * m_weights.variableDeclarationCost + 1 * m_weights.assignmentCost +
			2 * m_weights.identifierCost + 2 * m_weights.functionCallCost
	);
}

BOOST_AUTO_TEST_CASE(empty_for_loop) { BOOST_CHECK_EQUAL(codeSize("{ for {} 1 {} {} }"), 4); }

BOOST_FIXTURE_TEST_CASE(empty_for_loop_custom_weights, CustomWeightFixture)
{
	BOOST_CHECK_EQUAL(
		codeSize("{ for {} 1 {} {} }", m_weights),
		1 * m_weights.forLoopCost + 1 * m_weights.literalCost
	);
}

BOOST_AUTO_TEST_CASE(break_statement)
{
	BOOST_CHECK_EQUAL(codeSize("{ for {} 1 {} { break } }"), 6);
}
...
BOOST_FIXTURE_TEST_CASE(break_statement_custom_weights, CustomWeightFixture)
{
	BOOST_CHECK_EQUAL(
		...
		1 * m_weights.forLoopCost + 1 * m_weights.literalCost + 1 * m_weights.breakCost
	);
}

BOOST_AUTO_TEST_CASE(continue_statement)
{
	BOOST_CHECK_EQUAL(codeSize("{ for {} 1 {} { continue } }"), 6);
}
...
BOOST_FIXTURE_TEST_CASE(continue_statement_custom_weights, CustomWeightFixture)
{
	BOOST_CHECK_EQUAL(
		...
		1 * m_weights.forLoopCost + 1 * m_weights.literalCost + 1 * m_weights.continueCost
	);
}
...
{
	BOOST_CHECK_EQUAL(
		codeSize("{ for { let x := 0 } lt(x, 10) { x := add(x, 1) } { mstore(x, 1) } }"),
		10
	);
}
...
BOOST_FIXTURE_TEST_CASE(regular_for_loop_custom_weights, CustomWeightFixture)
{
	BOOST_CHECK_EQUAL(
		...
		1 * m_weights.forLoopCost + 1 * m_weights.variableDeclarationCost +
			1 * m_weights.assignmentCost + 3 * m_weights.functionCallCost +
			4 * m_weights.literalCost + 3 * m_weights.identifierCost +
			1 * m_weights.expressionStatementCost
	);
}

BOOST_AUTO_TEST_CASE(if_statement) { BOOST_CHECK_EQUAL(codeSize("{ if 1 {} }"), 3); }

BOOST_FIXTURE_TEST_CASE(if_statement_custom_weights, CustomWeightFixture)
{
	BOOST_CHECK_EQUAL(
		codeSize("{ if 1 {} }", m_weights),
		1 * m_weights.ifCost + 1 * m_weights.literalCost
	);
}

BOOST_AUTO_TEST_CASE(switch_statement_tiny)
{
	BOOST_CHECK_EQUAL(codeSize("{ switch calldatasize() case 0 {} }"), 4);
}

BOOST_AUTO_TEST_CASE(switch_statement_small)
{
	BOOST_CHECK_EQUAL(codeSize("{ switch calldatasize() case 0 {} default {} }"), 6);
}
...
BOOST_FIXTURE_TEST_CASE(switch_statement_small_custom_weights, CustomWeightFixture)
{
	BOOST_CHECK_EQUAL(
		...
		1 * m_weights.functionCallCost + 1 * m_weights.switchCost + 2 * m_weights.caseCost
	);
}

BOOST_AUTO_TEST_CASE(switch_statement_medium)
{
	BOOST_CHECK_EQUAL(codeSize("{ switch calldatasize() case 0 {} case 1 {} case 2 {} }"), 8);
}

BOOST_AUTO_TEST_CASE(switch_statement_large)
{
	BOOST_CHECK_EQUAL(
		codeSize("{ switch calldatasize() case 0 {} case 1 {} case 2 {} default {} }"),
		10
	);
}
...
BOOST_FIXTURE_TEST_CASE(switch_statement_large_custom_weights, CustomWeightFixture)
{
	BOOST_CHECK_EQUAL(
		...
		1 * m_weights.functionCallCost + 1 * m_weights.switchCost + 4 * m_weights.caseCost
	);
}
