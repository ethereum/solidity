...
std::shared_ptr<Block> parse(std::string const& _source, Dialect const& _dialect, ErrorReporter& errorReporter, Parser::DebugAttributeCache::Ptr debugAttributesCache = {})
{
...
BOOST_AUTO_TEST_CASE(ethdebug_debug_attributes_empty)
{
	...
	EVMDialectTyped const& dialect = EVMDialectTyped::instance(EVMVersion{});
	std::shared_ptr<Block> result = parse(sourceText, dialect, reporter, cache);
