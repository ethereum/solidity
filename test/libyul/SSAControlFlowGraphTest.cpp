...
{
public:
	SSACFGPrinter(SSACFG const& _cfg, SSACFG::BlockId _blockId): m_cfg(_cfg), m_functionIndex(0), m_sort(ForwardSSACFGTopologicalSort(_cfg)), m_liveness(_cfg)
	{
		printBlock(_blockId);
	}
	SSACFGPrinter(SSACFG const& _cfg, size_t _functionIndex, Scope::Function const& _function): m_cfg(_cfg), m_functionIndex(_functionIndex), m_sort(ForwardSSACFGTopologicalSort(_cfg)), m_liveness(_cfg)
	{
