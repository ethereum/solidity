...
object "DepositContract_541" {
    code {
        {
            ...
            let var_height := 0x00
            let var_height_1 := var_height
            for { }
            lt(var_height_1, 31)
            {
                if eq(var_height_1, not(0)) { panic_error_0x11() }
                var_height_1 := add(var_height_1, 0x01)
            }
            {
                let _2, _3 := storage_array_index_access_bytes(var_height_1)
                let _4 := extract_from_storage_value_dynamict_bytes32(sload(_2), _3)
                let _5, _6 := storage_array_index_access_bytes(var_height_1)
                let _7 := extract_from_storage_value_dynamict_bytes32(sload(_5), _6)
                let expr_mpos := mload(_1)
                let _8 := 0x20
                let _9 := sub(abi_encode_packed_bytes32_bytes32(add(expr_mpos, _8), _4, _7), expr_mpos)
                mstore(expr_mpos, add(_9, not(31)))
                finalize_allocation(expr_mpos, _9)
                let _10 := mload(_1)
                if iszero(staticcall(gas(), 2, _10, sub(abi_encode_bytes(expr_mpos, _10), _10), var_height, _8)) { revert_forward() }
                let _11 := mload(var_height)
                let _12, _13 := storage_array_index_access_bytes(checked_add_uint256(var_height_1))
                update_storage_value_bytes32_to_bytes32(_12, _13, _11)
            }
            let _14 := mload(_1)
            let _15 := datasize("DepositContract_541_deployed")
            codecopy(_14, dataoffset("DepositContract_541_deployed"), _15)
            return(_14, _15)
        }
    ...
    object "DepositContract_541_deployed" {
        ...
        data ".metadata" hex"a26469706673582212202f56134a5751d05d455a5737a74bc9c657eb244486a01b94998c4346d1a507d064736f6c637826302e382e372d646576656c6f702e323032312e372e372b636f6d6d69742e30306330666361660057"
    }
}
// ----