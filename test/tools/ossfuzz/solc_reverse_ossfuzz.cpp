...
//		}
		...
		solAssert(
			compilerOutputOpt.has_value(),
			"Contract could not be optimised via old code gen."
		);
		...
		solAssert(
			result == resultOpt,
			"Old and new code gen call results do not match."
		);
		...
		solAssert(
			...
			"Old and new code gen state do not match."
		);
		return 0;
	}
