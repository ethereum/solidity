...
class Test:
    ...
    def analyse(self):
        ...
        self.content = self.content.replace(self.source, "")
    ...
    def create_isoltest_call(self, line, test_comment):
        search = re.search(r'ABI_CHECK\(callContractFunction\(\"(.*)\",((.|\s)*)\), encodeArgs\(((.|\s)*)\)\);', line,
                           re.M | re.I)
        if search:
            result = search.group(1).strip() + ": " + self.eval_and_correct(
                search.group(2)) + " -> " + self.eval_and_correct(
                search.group(4))
            if test_comment:
                result += " # " + test_comment + " #"
            return result
        ...
        if search:
            result = search.group(1).strip() + ": " + self.eval_and_correct(
                search.group(2)) + " -> " + self.eval_and_correct("0x" + search.group(4))
            if test_comment:
                result += " # " + test_comment + " #"
            return result
        ...
        if search:
            result = search.group(1).strip() + " -> " + self.eval_and_correct(search.group(2))
            if test_comment:
                result += " # " + test_comment + " #"
            return result
    ...
    def extract(self):
        test_file = open("/tmp/end-to-end/" + self.name + ".sol", "w")
        if self.comment:
...
def main():
    cpp_file = open("/tmp/SolidityEndToEndTest.cpp", "r")
    test_name = ""
    test_content = ""
    inside_test = False
    tests = {}
