...
{
	Scope scope;
	InspectedInterpreter{_inspector, _state, _nameRepository, scope, _disableExternalCalls, _disableMemoryTrace}(_ast);
}
	...
	while (true)
	{
		...
		else if (input == "variables" || input == "v")
		{
			for (auto &&[yulName, val]: _variables)
				printVariable(m_nameRepository.requiredLabelOf(yulName), val);
			std::cout << std::endl;
		}
		else if (
		...
		)
		{
			...
			for (auto &&[yulName, val]: _variables)
			{
				auto const yulStr = m_nameRepository.requiredLabelOf(yulName);
				if (yulStr == varname)
				{
...
{
	InspectedExpressionEvaluator ev(m_inspector, m_state, m_nameRepository, *m_scope, m_variables, m_disableExternalCalls, m_disableMemoryTrace);
