...
protected:
	...
	)
	{
		bytes const& compiled =
			s_compiledWallet.init([&] { return compileContract(walletCode, "Wallet"); });
...
BOOST_AUTO_TEST_CASE(creation)
{
	...
	BOOST_REQUIRE(
		callContractFunction("isOwner(address)", h256(~0)) ==
		(v2 ? encodeArgs() : encodeArgs(false))
	);
}
...
BOOST_AUTO_TEST_CASE(add_owners)
{
	...
	BOOST_REQUIRE(
		callContractFunction("addOwner(address)", h256(account(1), h256::AlignRight)) ==
		encodeArgs()
	);
	BOOST_REQUIRE(
		callContractFunction("isOwner(address)", h256(account(1), h256::AlignRight)) ==
		encodeArgs(true)
	);
	...
	BOOST_REQUIRE(
		callContractFunction("isOwner(address)", h256(originalOwner, h256::AlignRight)) ==
		encodeArgs(true)
	);
	BOOST_REQUIRE(
		callContractFunction("isOwner(address)", h256(account(1), h256::AlignRight)) ==
...
BOOST_AUTO_TEST_CASE(change_owners)
{
	...
	BOOST_REQUIRE(
		callContractFunction("changeOwner(address,address)", h256(0x12), h256(0x13)) == encodeArgs()
	);
...
BOOST_AUTO_TEST_CASE(remove_owner)
{
	...
	for (unsigned i = 0; i < 10; ++i)
		if (i % 2 == 1)
			BOOST_REQUIRE(
				callContractFunction("removeOwner(address)", h256(0x12 + i)) == encodeArgs()
			);
	// check the result
	for (unsigned i = 0; i < 10; ++i)
		BOOST_REQUIRE(
			callContractFunction("isOwner(address)", h256(0x12 + i)) == encodeArgs(i % 2 == 0)
		);
	// add them again
	for (unsigned i = 0; i < 10; ++i)
		if (i % 2 == 1)
			BOOST_REQUIRE(
				callContractFunction("addOwner(address)", h256(0x12 + i)) == encodeArgs()
			);
...
BOOST_AUTO_TEST_CASE(multisig_value_transfer)
{
	...
	BOOST_REQUIRE(
		callContractFunction("addOwner(address)", h256(account(2), h256::AlignRight)) ==
		encodeArgs()
	);
	BOOST_REQUIRE(
		callContractFunction("addOwner(address)", h256(account(3), h256::AlignRight)) ==
		encodeArgs()
	);
	...
	sendEther(account(1), 10 * ether);
	m_sender = account(1);
	auto ophash = callContractFunction(
		"execute(address,uint256,bytes)",
		h256(destination, h256::AlignRight),
		100,
		0x60,
		0x00
	);
...
BOOST_AUTO_TEST_CASE(revoke_addOwner)
{
	...
	h256 opHash = util::keccak256(
		FixedHash<4>(util::keccak256("addOwner(address)")).asBytes() + h256(0x33).asBytes()
	);
