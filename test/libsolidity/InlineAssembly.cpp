...
{
	return successParse(_source, true, _allowWarnings, _language, AssemblyStack::Machine::EVM) &&
...
void parsePrintCompare(string const& _source, bool _canWarn = false)
{
	...
	string expectation = "object \"object\" {\n    code " +
		boost::replace_all_copy(_source, "\n", "\n    ") + "\n}\n";
...
#define CHECK_PARSE_ERROR(text, type, substring) CHECK_ERROR(text, false, type, substring, false)

#define CHECK_PARSE_WARNING(text, type, substring) CHECK_ERROR(text, false, type, substring, false)

#define CHECK_ASSEMBLE_ERROR(text, type, substring) CHECK_ERROR(text, true, type, substring, false)
...
#define SUCCESS_STRICT(text)                                                         \
	do                                                                               \
	{                                                                                \
		successParse((text), false, false, AssemblyStack::Language::StrictAssembly); \
	} while (false)
...
BOOST_AUTO_TEST_CASE(selfdestruct) { BOOST_CHECK(successParse("{ selfdestruct(0x02) }")); }
...
BOOST_AUTO_TEST_CASE(constants) { BOOST_CHECK(successParse("{ pop(mul(7, 8)) }")); }
...
BOOST_AUTO_TEST_CASE(functional_assignment) { BOOST_CHECK(successParse("{ let x := 2 x := 7 }")); }
...
{
	CHECK_PARSE_ERROR(
		"{ switch 42 case 1 {} case 1 {} default {} }",
		DeclarationError,
...
{
	CHECK_PARSE_ERROR(
		"{ switch 42 case mul(1, 2) {} case 2 {} default {} }",
		ParserError,
		"Literal expected."
	);
}
...
{
	CHECK_PARSE_ERROR(
		"{ switch 42 case 1 mul case 2 {} default {} }",
		ParserError,
...
{
	BOOST_CHECK(
		successParse("{ function f(a) -> b {} function g(a, b, c) {} function x() { g(1, 2, "
					 "f(mul(2, 3))) x() } }")
	);
}
...
BOOST_AUTO_TEST_CASE(name_clashes)
{
	CHECK_PARSE_ERROR(
		"{ let g := 2 function g() { } }",
		DeclarationError,
		"Variable name g already taken in this scope"
	);
}
...
{
	CHECK_PARSE_ERROR(
		"{ let x function f() -> a, b {} 123, x := f() }",
		ParserError,
		"Variable name must precede \",\" in multiple assignment."
	);
	CHECK_PARSE_ERROR(
		"{ let x function f() -> a, b {} x, 123 := f() }",
		ParserError,
...
BOOST_AUTO_TEST_CASE(print_instructions) { parsePrintCompare("{ pop(7) }"); }

BOOST_AUTO_TEST_CASE(print_subblock) { parsePrintCompare("{ { pop(7) } }"); }

BOOST_AUTO_TEST_CASE(print_functional) { parsePrintCompare("{ let x := mul(sload(0x12), 7) }"); }
...
BOOST_AUTO_TEST_CASE(print_if) { parsePrintCompare("{ if 2 { pop(mload(0)) } }"); }
...
BOOST_AUTO_TEST_CASE(oversize_string_literals)
{
	CHECK_ASSEMBLE_ERROR(
		"{ let x := \"123456789012345678901234567890123\" }",
		TypeError,
...
BOOST_AUTO_TEST_CASE(revert) { BOOST_CHECK(successAssemble("{ revert(0, 0) }")); }
...
BOOST_AUTO_TEST_CASE(function_calls)
{
	...
	BOOST_CHECK(
		successAssemble("{ function f(a, b) -> x, y { x := b y := a } let a, b := f(2, 3) }")
	);
...
BOOST_AUTO_TEST_CASE(switch_statement)
{
	...
	BOOST_CHECK(
		successAssemble("{ let a := 3 switch a case 1 { a := 1 } case 2 { a := 5 } a := 9}")
	);
	BOOST_CHECK(
		successAssemble("{ let a := 2 switch calldataload(0) case 1 { a := 1 } case 2 { a := 5 } }")
	);
}
...
BOOST_AUTO_TEST_CASE(keccak256) { BOOST_CHECK(successAssemble("{ pop(keccak256(0, 0)) }")); }
...
BOOST_AUTO_TEST_CASE(shift_constantinople_warning)
{
	...
	CHECK_PARSE_WARNING(
		"{ shr(10, 32) }",
		TypeError,
		"The \"shr\" instruction is only available for Constantinople-compatible VMs"
	);
	CHECK_PARSE_WARNING(
		"{ sar(10, 32) }",
		TypeError,
		"The \"sar\" instruction is only available for Constantinople-compatible VMs"
	);
}
