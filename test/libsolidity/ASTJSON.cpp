...
BOOST_AUTO_TEST_CASE(importAST)
{
	...
	map<string, unsigned> sourceIndices;
	sourceIndices["a"] = 0;
	Json::Value originalJson = ASTJsonConverter(false, sourceIndices).toJson(c.ast("a"));
	...
	// Note: this bytecode comparison will only succeed for contracts don't import/inherit other contracts.
	// In order to test more complicated contracts, the bytecodeSansMetadata-function needs to be extended to
	// also remove metadata-bytecode not only from the end but also from the middle of a given string.
