...
)
{
	...
	{
		// This must be a vector of unique_ptrs because Boost.Test keeps the equivalent of a
		// string_view to the filename that is passed in. If the strings were stored directly in the
		// vector, pointers/references to them would be invalidated on reallocation.
				...
				BOOST_REQUIRE_NO_THROW(
					{
						...
						{
							BOOST_ERROR(
								"Exception during extracted test: "
...
test_suite* init_unit_test_suite(int /*argc*/, char* /*argv*/[])
{
	...
	bool disableSemantics =
		!solidity::test::EVMHost::getVM(solidity::test::CommonOptions::get().evmonePath.string());
	if (disableSemantics)
	{
	...
	for (auto const& ts: g_interactiveTestsuites)
	{
		...
		solAssert(
			...
			std::string("no ") + ts.title + " tests found"
		);
	}
...
// Because we want to have customized initialization function and support shared boost libraries at
// the same time, we are forced to customize the entry point. see:
// https://www.boost.org/doc/libs/1_67_0/libs/test/doc/html/boost_test/adv_scenarios/shared_lib_customizations/init_func.html
