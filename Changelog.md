 ...
 * Yul Optimizer: LoopInvariantCodeMotion can move `extcodehash`, `extcodesize`, `returndatasize`, `balance` and  `selfbalance`, if the loop does not invalidate the blockchain state.
 * Yul Optimizer: LoopInvariantCodeMotion can move `sload` if the loop does not invalidate the storage.
